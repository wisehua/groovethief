        ...
        std::size_t bytes_read(
                asio::read(s,
                           asio::mutable_buffers_1(
                                   asio::mutable_buffer(&octet, sizeof(octet)))));
...
struct do_read<port_t>
{
    ...
    port_t operator()(SyncReadStream & s)
    {
        ...
        std::size_t port_read(
                    asio::read(s,
                               asio::mutable_buffers_1(
                                       asio::mutable_buffer(&port, sizeof(port)))));
...
struct do_write<octet_t>
{
    ...
    {
        std::size_t wrote(
                asio::write(s, asio::const_buffers_1(
                        asio::const_buffer(&o, sizeof(o)))));
        if (wrote != sizeof(o))
            abort();
    }
};
...
struct do_write<port_t>
{
    ...
    void operator()(SyncWriteStream & s, port_t p)
    {
        ...
        std::size_t wrote(
                asio::write(s, asio::const_buffers_1(
                        asio::const_buffer(&nhost, sizeof(nhost)))));
