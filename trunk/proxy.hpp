...
private:
    ...
    std::auto_ptr<mitm_manager> manager_;
};
...
void echoer(ip::tcp::socket & in, ip::tcp::socket & out, std::string const& debug_message)
{
    ...
    while (true) {
        ...
        std::size_t bytes_read(
                in.read_some(
                    asio::mutable_buffers_1(asio::mutable_buffer(&buffer.front(), buffer.size())),
                    ec));
        ...
        std::size_t bytes_wrote(
                asio::write(
                        out,
                        asio::const_buffers_1(asio::const_buffer(&buffer.front(), bytes_read)),
                        asio::transfer_all(),
                        ec));
